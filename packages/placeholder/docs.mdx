...
Use `<Placeholder />` to render [skeleton loading states](https://uxdesign.cc/what-you-should-know-about-skeleton-screens-a820c45a571a). The component passed to the render prop, named `Box` below, accepts `prose`, `lines` and `repeat` as unique props. All other props are spread to the elements `style` prop.
