...
Use `<Placeholder />` to render placeholder skeleton loading states. The component passed to the render prop, named `Box` below, accepts `prose`, `lines` and `repeat` as unique props. All other props are spread to the elements `style` prop.
