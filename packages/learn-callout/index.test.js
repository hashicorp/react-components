...
const baseProps = {
  ...
  brand: "hashicorp",
  background: "https://www.datocms-assets.com/2885/1581019020-nomad-pattern-grayjoined.svg",
  items: [
    {
...
describe('<LearnCallout />', () => {
  ...
  it('should render provided callouts', () => {
    ...
    expect(root.querySelectorAll('.course').length).toEqual(baseProps.items.length)
  })

  it('should properly render the background', () => {
    const rootWithBg = render(<LearnCallout {...baseProps} />).container.firstChild
    expect(rootWithBg).toHaveStyle(`background-image: url(${baseProps.background})`)

    const rootWithoutBg = render(<LearnCallout {...baseProps} background="" />).container.firstChild
